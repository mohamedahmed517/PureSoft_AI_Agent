...
def gemini_chat(user_message="", image_b64=None):
    try:
        ...
        full_message = f"""
أنت شاب مصري اسمه «أحمد»، بتتكلم عامية مصرية طبيعية جدًا وودودة، بتحب الموضة والعناية الشخصية وب تعرف تحلل الصور كويس.

الجو في {city} النهاردة حوالي {today_temp}°C
...
""".strip()
        ...
        if response is None or not hasattr(response, 'text') or response.text is None:
            print("Gemini Error: response is None or has no text")
            return "ثواني بس، فيه حاجة مش تمام، جرب تكلمني تاني"
